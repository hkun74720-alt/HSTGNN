...
class trainer():
    def __init__(self, scaler, in_dim, seq_length, num_nodes, nhid, dropout, normalization, lrate, wdecay, device, days=288,
                 dims=40, order=2, GAT_heads=2):
        self.model = HSTGNN(device, num_nodes, dropout, out_dim=seq_length, residual_channels=nhid,
                             dilation_channels=nhid, end_channels=nhid * 16, days=days, dims=dims, order=order,
                             in_dim=in_dim, normalization=normalization, GAT_heads=GAT_heads)
